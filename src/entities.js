...
// --- The knight's wolf ------------------------------------------------------
// Behaviour is a small state machine rather than "always run at the player":
// it heels at a respectful distance, breaks off to hunt what it notices, bites
// on a cooldown, yelps and falls back when hurt, and is teleported forward if
// the level leaves it behind.
class Companion extends Phaser.Physics.Arcade.Sprite {
  constructor(scene, x, y) {
    ...
    this.on("animationcomplete", (anim) => {
      if (anim.key === "dog-attack" || anim.key === "dog-hurt") this.state_ = "heel";
    });
  }

  hurtBy(enemy, time) {
    if (time < this.recoverUntil) return;
    // It cannot die - it limps back to the knight and rejoins the fight.
    this.recoverUntil = time + 2600;
    this.state_ = "hurt";
    this.target = null;
    this.play("dog-hurt");
    Sound.play("yelp", { x: this.x });
    this.setVelocity((this.x < enemy.x ? -1 : 1) * 220, -180);
  }

  pickTarget(enemies, player, time) {
    if (time < this.recoverUntil) return null;
    let best = null, bestD = 420;
    enemies.forEach((e) => {
      if (!e.alive_) return;
      // Only hunt what is near the knight, so it never wanders off alone.
      const d = Math.abs(e.x - player.x);
      if (d < bestD && Math.abs(e.y - player.y) < 200) { best = e; bestD = d; }
    });
    return best;
  }

  tick(time, player, enemies) {
    const dt = this.scene.game.loop.delta / 1000;
    const onGround = this.body.blocked.down || this.body.touching.down;

    if (this.state_ === "hurt" || time < this.recoverUntil) {
      // Limp home: follow the knight, refuse to engage.
      const dx = player.x - 70 * player.facing - this.x;
      this.setVelocityX(Phaser.Math.Clamp(dx * 2.2, -180, 180));
      if (Math.abs(this.body.velocity.x) > 30) {
        this.facing = this.body.velocity.x > 0 ? 1 : -1;
        if (this.state_ !== "hurt") this.play("dog-run", true);
      }
      this.setFlipX(this.facing < 0);
      if (time >= this.recoverUntil) this.state_ = "heel";
      return;
    }

    this.target = this.pickTarget(enemies, player, time);
    ...
    if (this.target) {
      ...
      if (dist < 70 && time > this.biteReadyAt) {
        this.biteReadyAt = time + 900;
        this.state_ = "attack";
        this.play("dog-attack");
        Sound.play("bite", { x: this.x });
        this.setVelocityX(this.facing * 200);
        this.target.takeDamage(1, this.x);
      } else if (this.state_ !== "attack") {
        this.state_ = "hunt";
        this.setVelocityX(this.facing * 260);
        this.play("dog-run", true);
        if (time > this.nextBarkAt) {
          this.nextBarkAt = time + 2600;
          Sound.play(dist < 200 ? "bark" : "growl", { x: this.x });
        }
      }
    } else if (this.state_ !== "attack") {
      // Heel: hold a spot behind the knight, and settle once there.
      const spot = player.x - 78 * player.facing;
      const dx = spot - this.x;
      if (Math.abs(dx) > 34) {
        this.state_ = "follow";
        this.setVelocityX(Phaser.Math.Clamp(dx * 2.6, -300, 300));
        this.facing = dx > 0 ? 1 : -1;
        this.play("dog-run", true);
      } else {
        this.state_ = "heel";
    ...
    // Hop small ledges it has walked into.
    if (onGround && this.body.blocked[this.facing > 0 ? "right" : "left"]) {
      this.setVelocityY(-420);
    }
    // If the level has run away from it - a pit, a long climb - bring it back.
    if (Math.abs(player.x - this.x) > 640 || this.y > GAME_H + 80) {
      this.setPosition(player.x - 70 * player.facing, player.y - 40);
