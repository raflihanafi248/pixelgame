...
const GameState = {
  ...
  // What the thing in the lair will weigh at the end. Both survive a death
  // and a change of chapter, and both are wiped when a new game starts - you
  // cannot carry last run's knowledge into this one.
...
const PANEL_CREDITS = [
  ...
  ["What is in the lair", "free_cthulu v1"],
  ["", "all seven of its animations, all seven a move"],
...
// is half of what the thing in the lair is listening for at the end.
...
class BootScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.spritesheet("cthulhu", ASSET_DATA.cthulhu, { frameWidth: 192, frameHeight: 112 });
    this.load.spritesheet("cthulhu_bolt", ASSET_DATA.cthulhu_bolt, { frameWidth: 42, frameHeight: 28 });
    this.load.spritesheet("cthulhu_rift", ASSET_DATA.cthulhu_rift, { frameWidth: 48, frameHeight: 96 });
    ...
    // It speaks once, at the end, and needs a face for it.
    this.load.image("portrait_cthulhu", ASSET_DATA.portrait_cthulhu);
  ...
  create() {
    ...
    // The boss. Frame ranges come straight out of tools/pack_cthulhu.py, which
    // prints them when it packs the sheet: idle 0-14, walk 15-26, fly 27-32,
    // 1atk 33-39, 2atk 40-48, hurt 49-53, death 54-64.
    A.create({ key: "cth-idle", frames: A.generateFrameNumbers("cthulhu", { start: 0, end: 14 }), frameRate: 10, repeat: -1 });
    A.create({ key: "cth-walk", frames: A.generateFrameNumbers("cthulhu", { start: 15, end: 26 }), frameRate: 12, repeat: -1 });
    A.create({ key: "cth-fly", frames: A.generateFrameNumbers("cthulhu", { start: 27, end: 32 }), frameRate: 12, repeat: -1 });
    A.create({ key: "cth-cast", frames: A.generateFrameNumbers("cthulhu", { start: 33, end: 39 }), frameRate: 11 });
    A.create({ key: "cth-lash", frames: A.generateFrameNumbers("cthulhu", { start: 40, end: 48 }), frameRate: 13 });
    // hurt frame 0 is the whole body blown out to white - exactly a hit flash,
    // so the flinch is that frame and the first of the flare behind it.
    A.create({ key: "cth-flinch", frames: A.generateFrameNumbers("cthulhu", { frames: [49, 50] }), frameRate: 16 });
    A.create({ key: "cth-scream", frames: A.generateFrameNumbers("cthulhu", { start: 50, end: 53 }), frameRate: 7, repeat: -1 });
    // The collapse, split in two: it sinks onto the stone and stays there to
    // ask its question, and only lets go once it has an answer.
    // Frame 56 is the one the whole ending hangs on: down on the stone, wings
    // collapsed, head still up. It goes down to it, holds it with a slow
    // heave, and only carries on into the rest of the collapse if you let it.
    A.create({ key: "cth-sink", frames: A.generateFrameNumbers("cthulhu", { start: 54, end: 56 }), frameRate: 6 });
    A.create({ key: "cth-kneel", frames: A.generateFrameNumbers("cthulhu", { frames: [56, 56, 57, 57] }), frameRate: 2, repeat: -1 });
    A.create({ key: "cth-melt", frames: A.generateFrameNumbers("cthulhu", { start: 57, end: 64 }), frameRate: 5 });
    A.create({ key: "cth-stand", frames: A.generateFrameNumbers("cthulhu", { frames: [56, 55, 54] }), frameRate: 8 });

    A.create({ key: "bolt-fly", frames: A.generateFrameNumbers("cthulhu_bolt", { start: 0, end: 1 }), frameRate: 14, repeat: -1 });
    A.create({ key: "bolt-burst", frames: A.generateFrameNumbers("cthulhu_bolt", { start: 2, end: 3 }), frameRate: 14 });
    // The crack, the rise, a beat of it standing there, and the sink. The
    // repeated frame is the hold: long enough to be a hazard you walk around.
    A.create({ key: "rift-open", frames: A.generateFrameNumbers("cthulhu_rift", { frames: [0, 1, 2, 3, 4, 4, 4, 4, 5] }), frameRate: 8 });
    for (const npc of CAST_FOLK) {
...
class TitleScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(LX, 152, "The forest guardian, and the oldest lie in the wood", {
...
class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    if (level.dark) {
      ...
      // One off-list image does all the erasing. Giving it a scale per light
      // is the only way to have lights of different sizes: RenderTexture.erase
      // takes no size of its own, but it does honour a game object's.
      this.lightStamp = this.make.image({ key: "light", add: false }).setOrigin(0);
      this.events.once("shutdown", () => this.lightStamp.destroy());
    }
    this.buildSky();
    this.buildWeather();
  }
  ...
  buildProps() {
    ...
    for (const t of this.level.torches || []) {
      ...
      this.lanterns.push({ x: t, y: GROUND_Y - 76, r: this.level.torchLight ?? 1 });
    }
  }
  ...
  spawnActors() {
    ...
    // Everything the boss throws lives in these two. Both are created before
    // any collider touches them - an overlap registered against a group that
    // does not exist yet is what froze the scene the last time.
    this.bolts = this.physics.add.group({ allowGravity: false, maxSize: 24 });
    this.rifts = this.physics.add.group({ allowGravity: false, maxSize: 12 });
    ...
    this.physics.add.overlap(this.player, this.bolts, (pl, bolt) => {
      this.burstBolt(bolt);
      pl.hurt(1, bolt.x);
    });
    this.physics.add.overlap(this.player, this.rifts, (pl, rift) => {
      // The crack opening is the warning; only what comes out of it bites.
      if (rift.armed) pl.hurt(1, rift.x);
    });

    if (this.level.boss === "cthulhu") {
      this.boss = new Cthulhu(this, this.levelWidth - 300);
      this.physics.add.overlap(this.player, this.boss, (pl, boss) => {
        if (boss.alive_ && boss.woken) pl.hurt(1, boss.x);
      });
  ...
  buildHud() {
    ...
    if (this.level.boss) {
      ...
      this.bossBar = this.add.rectangle(GAME_W / 2 - 258, GAME_H - 34, 516, 14, 0x74c23a)
        .setOrigin(0, 0.5).setScrollFactor(0).setDepth(61);
      this.bossName = this.add.text(GAME_W / 2, GAME_H - 56, "WHAT THEY CALL THE DRAGON", {
        fontFamily: FONT, fontSize: "14px", color: "#b6ff7a",
      }).setOrigin(0.5).setScrollFactor(0).setDepth(61);
      // Nothing down there is fighting you yet. The bar arrives when it does.
  ...
  playerHitCheck(player) {
    ...
    // The boss's own bounds are the whole 192x112 frame times its scale -
    // wings, lash and all - which would let a sword land from two hundred
    // pixels away. Its physics body is the part you can actually reach.
    if (this.boss && this.boss.alive_ && this.boss.woken) {
      const b = this.boss.body;
      const torso = new Phaser.Geom.Rectangle(b.x, b.y, b.width, b.height);
      if (Phaser.Geom.Rectangle.Overlaps(box, torso)) {
        this.boss.takeDamage(damage, player.x);
        hitSomething = true;
      }
    }
    this.bolts.getChildren().forEach((bolt) => {
      if (bolt.active && Phaser.Geom.Rectangle.Overlaps(box, bolt.getBounds())) {
        this.burstBolt(bolt);
        hitSomething = true;
      }
    });
  ...
  // The lance, cut straight out of its own 1atk frames. It is drawn pointing
  // right, so the sprite's rotation is simply the direction it is travelling.
  throwBolt(x, y, angle) {
    const bolt = this.bolts.get(x, y, "cthulhu_bolt");
    if (!bolt) return;
    bolt.setActive(true).setVisible(true).setAlpha(1);
    bolt.body.enable = true;
    bolt.setScale(1.8).setDepth(46).setRotation(angle);
    bolt.body.setSize(22, 12);
    bolt.body.setOffset(10, 8);
    bolt.play("bolt-fly");
    bolt.setVelocity(Math.cos(angle) * 430, Math.sin(angle) * 430);
  }

  // Hit something: the pack's own last two 1atk frames are the lance coming
  // apart, so that is exactly what plays.
  burstBolt(bolt) {
    if (!bolt.active) return;
    bolt.body.enable = false;
    bolt.setVelocity(0, 0);
    Sound.play("boltHit", { x: bolt.x });
    bolt.play("bolt-burst");
    bolt.once("animationcomplete", () => this.retireBolt(bolt));
  }
  ...
  // The floor splits and the wood's roots - which is what the villagers have
  // always told each other they are - come up through it.
  openRift(x) {
    const rift = this.rifts.create(x, GROUND_Y + 6 - RIFT_H * RIFT_SCALE / 2,
                                   "cthulhu_rift");
    if (!rift) return;
    rift.setScale(RIFT_SCALE).setDepth(16);
    rift.body.setAllowGravity(false);
    rift.body.setSize(30, 62);
    rift.body.setOffset(9, 32);
    rift.armed = false;
    rift.play("rift-open");
    Sound.play("rift", { x });

    const light = { x, y: GROUND_Y - 40, r: 0.75 };
    this.lanterns.push(light);
    const halo = this.add.image(x, GROUND_Y - 30, "light")
      .setTint(0x8ce84a).setScale(0.55).setAlpha(0.22).setDepth(46)
      .setBlendMode(Phaser.BlendModes.ADD);
    // Two frames of a glowing crack before anything comes out of it: that is
    // the whole warning, and it is enough to step off.
    this.time.delayedCall(230, () => { if (rift.active) rift.armed = true; });
    rift.once("animationcomplete", () => {
      Phaser.Utils.Array.Remove(this.lanterns, light);
      halo.destroy();
      rift.destroy();
    });
  }
  ...
  callBroodling(x, y) {
    this.enemies.add(new Broodling(this, x, y));
  }

  broodCount() {
    return this.enemies.getChildren().filter(
      (e) => e instanceof Broodling && e.alive_).length;
  }

  // Where it lands out of a dive: a ring of grit and a flash off the stone.
  groundBurst(x) {
    const ring = this.add.image(x, GROUND_Y - 8, "light")
      .setTint(0x9cf05a).setScale(0.2).setAlpha(0.85).setDepth(46)
      .setBlendMode(Phaser.BlendModes.ADD);
    this.tweens.add({ targets: ring, scaleX: 1.1, scaleY: 0.45, alpha: 0,
                      duration: 420, onComplete: () => ring.destroy() });
    for (let i = 0; i < 12; i++) {
      const g = this.add.image(x, GROUND_Y - 6, "particle")
        .setTint(0x6f8f5a).setScale(Phaser.Math.FloatBetween(1, 2.4)).setDepth(46);
      this.tweens.add({
        targets: g, x: x + Phaser.Math.Between(-190, 190),
        y: GROUND_Y - Phaser.Math.Between(20, 130), alpha: 0,
        duration: Phaser.Math.Between(340, 700), ease: "Quad.easeOut",
  ...
  onBossWakes() {
    ...
    this.cameras.main.flash(500, 34, 78, 26);
  }

  showBossBar(on) {
    this.bossBar?.setVisible(on);
    this.bossBarBg?.setVisible(on);
  ...
  // It has stopped. Everything stops with it: the music, the weather,
  ...
  onBossKneels() {
    ...
    // the dialogue box while it asked its question.
    this.showBossBar(false);
    ...
    this.time.delayedCall(1700, () => {
      ...
      const question = LAST_QUESTION.map((line) => {
        if (!line.options) return line;
        return { ...line,
                 options: line.options.filter((o) => !o.secret || GameState.knowsEverything()) };
      });
      this.dialogue.start(question, (answer) => this.answerBoss(answer));
    });
  }

  answerBoss(answer) {
    const reply = LAST_REPLY[answer] || LAST_REPLY.truth;
    this.dialogue.start(reply, () => this.finishBoss(answer));
  }
  ...
  completeLevel() {
    ...
    this.time.delayedCall(850, () => {
      ...
      // The last chapter does not end at a gate - it ends at the boss's
  ...
  update(time) {
    ...
    if (Phaser.Input.Keyboard.JustDown(this.keys.talk)) {
      ...
      if (who) {
        ...
        if (who.def.shop) {
          ...
          Sound.play("select", { x: who.x });
          // Heard, not merely met: the lair weighs whether you sat through
    ...
    if (this.boss) {
      ...
      // The score follows the fight: every point of its health it loses lets
      // another layer of the theme in.
      if (this.boss.alive_) Sound.setIntensity(1 - this.boss.hp / this.boss.maxHp);
    }

    this.bolts.getChildren().forEach((bolt) => {
      if (!bolt.active) return;
      if (bolt.x < 0 || bolt.x > this.levelWidth || bolt.y < -100 || bolt.y > GAME_H + 100) {
        this.retireBolt(bolt);
      }
    });
  ...
  drawDarkness() {
    this.darkness.clear();
    this.darkness.fill(0x05060f, this.level.gloom ?? 0.88);
    // The knight carries his own light, and in the lair it is a small one.
    this.stampLight(this.player.x, this.player.y, this.level.lightScale ?? 1);
    for (const l of this.lanterns) this.stampLight(l.x, l.y, l.r ?? 1);
  }

  stampLight(x, y, scale) {
    const cam = this.cameras.main;
    const size = LIGHT_SIZE * scale;
    const lx = x - cam.scrollX - size / 2;   // erase takes a corner, not a centre
    if (lx < -size || lx > GAME_W) return;
    this.lightStamp.setScale(scale);
    this.darkness.erase(this.lightStamp, lx, y - cam.scrollY - size / 2);
  }
}
...
class EndingScene extends Phaser.Scene {
  ...
  // One scene, three endings. Which one arrives is decided at the boss's
